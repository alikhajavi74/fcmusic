...
class _MyAppState extends State<MyApp> with WidgetsBindingObserver {
  ...
  Future<void> _init() async {
    ...
    _audioPlayer.playbackEventStream.listen((event) {}, onError: (exception, stackTrace) {
      print('A stream error occurred: $exception');
    });
    try {
      // Preloading audio is not currently supported on Linux.
      await _audioPlayer.setAudioSource(_playlist, preload: kIsWeb || defaultTargetPlatform != TargetPlatform.linux);
    } catch (e) {
  ...
  Stream<PositionData> get _positionDataStream => Rx.combineLatest3<Duration, Duration, Duration?, PositionData>(_audioPlayer.positionStream, _audioPlayer.bufferedPositionStream, _audioPlayer.durationStream, (currentPosition, bufferedPosition, totalDuration) => PositionData(currentPosition, bufferedPosition, totalDuration ?? Duration.zero));
            ...
            children: [
              ...
              StreamBuilder<PositionData>(
                ...
                builder: (context, snapshot) {
                  ...
                  return SeekBar(
                    duration: positionData?.totalDuration ?? Duration.zero,
                    position: positionData?.currentPosition ?? Duration.zero,
                  ...
                  StreamBuilder<LoopMode>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      return IconButton(
                        icon: icons[index],
                        onPressed: () {
                          _audioPlayer.setLoopMode(cycleModes[(cycleModes.indexOf(loopMode) + 1) % cycleModes.length]);
                        },
                      );
                    },
                  ),
